    ...
    if not df_filtrado.empty:
        ...
        with st.form("informe_form"):
            ...
            with tabs[2]:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    # Usar number_input si se esperan valores numéricos; en este caso se usa text_input con valores por defecto
                    ...
                    with st.expander(f"Completar datos del Área {i}", expanded=True):
                        ...
                        with col1:
                            ...
                            tbs = st.number_input(f"Temperatura bulbo seco (°C) - Área {i}", value=0.0, step=0.1,
                                                  key=f"tbs_{i}")
                            tg = st.number_input(f"Temperatura globo (°C) - Área {i}", value=0.0, step=0.1,
                                                 key=f"tg_{i}")
                        ...
                        with col4:
                            ...
                            condiciones_disconfort = st.text_input(
                                f"Otras condiciones de disconfort térmico - Área {i}", key=f"cond_{i}")
