...
from utils.helpers import autenticar_usuario, get_ct, precompletar_campos_ct, interpreter_pmv
...
if not checkear_session():
    ...
    def main():
        ...
        if st.button("Buscar"):
            ...
            # Filtrar el CSV principal (para otros fines, como generar el informe)
        ...
        if not df_info_cuv.empty:
            ...
            with st.form("informe_form"):
                ...
                motivo_evaluacion = st.selectbox("Motivo de evaluación",
                                                 options=["Seleccione...", "Programa anual", "Solicitud empresa",
                                                          "Fiscalización"],
                                                 index=0)
                ...
                for i, area_tab in enumerate(area_tabs, start=1):
                    with area_tab:
                        ...
                        area_sector = st.selectbox(f"Área {i}",
                                                   options=["Seleccione...", "Linea de cajas", "Sala de venta",
                                                            "Bodega", "Recepción"],
                                                   key=f"area_{i}")
                        ...
                        techumbre = st.pills(
                            f"**La techumbre del área evaluada, cuenta con materiales aislantes térmico tales como: Policarbonatos extendidos, lana mineral, gomas, espuma de poliuretano, entre otros.**",
                            key=f"techumbre_{i}", options=["Sí", "No"])
                        obs_techumbre = st.text_input(f"Observación techumbre - obligatorio en caso de disconformidad",
                                                      key=f"obs_techumbre_{i}")
                        st.write("")
                        paredes = st.pills(
                            f"**En las paredes del área evaluada,donde  incida directamente el sol, cuentan con material aislante térmicos tales como: Policarbonatos extendidos, lana mineral, gomas, espuma de poliuretano, o construcción de hormigón entre otros (especificar)**",
                            key=f"paredes_{i}", options=["Sí", "No"])
                        obs_paredes = st.text_input(f"Observación paredes - obligatorio en caso de disconformidad",
                                                    key=f"obs_paredes_{i}")
                        st.write("")
                        ventanales = st.pills(
                            f"**Los ventanales del área en los cuales incide directamente el sol, cuentan con algún tipo de material aislante, tales como laminas de protección solar, cortinas, gigantografías que proporcionen un apantallamiento.**",
                            key=f"ventanales_{i}", options=["Sí", "No"])
                        obs_ventanales = st.text_input(
                            f"Observación ventanales - obligatorio en caso de disconformidad",
                            key=f"obs_ventanales_{i}")
                        st.write("")
                        aire_acond = st.pills(
                            f"**El área cuenta con sistema de aire acondicionado y/o enfriador de aire, que proporcione movimiento de aire frío, con el fin de mejorar el confort para los trabajadores. Indicar temperatura de funcionamiento (°C).**",
                            key=f"aire_{i}", options=["Sí", "No"])
                        obs_aire_acond = st.text_input(
                            f"Observaciones aire acondicionado - obligatorio especificar cantidad y operatividad.",
                            key=f"obs_aire_{i}")
                        st.write("")
                        ventiladores = st.pills(
                            f"**El área cuenta con ventiladores que proporcionan movimiento de aire, con el fin de mejorar el confort para los trabajadores.**",
                            key=f"venti_{i}", options=["Sí", "No"])
                        obs_ventiladores = st.text_input(
                            f"Observaciones ventiladores - obligatorio especificar cantidad y operatividad.",
                            key=f"obs_venti_{i}")
                        st.write("")
                        inyeccion_extrac = st.pills(f"**El área cuenta con inyección y/o extracción de aire.**",
                                                    key=f"inye_{i}", options=["Sí", "No"])
                        obs_inyeccion = st.text_input(
                            f"Observaciones inyección/extracción de aire - obligatorio especificar cantidad y operatividad.",
                            key=f"obs_inye_{i}")
                        st.write("")
                        ventanas = st.pills(
                            f"**El área evaluada cuenta con ventanas que permiten la entrada de aire fresco y la salida de aire caliente, favoreciendo así la ventilación natural.**",
                            key=f"ventana_{i}", options=["Sí", "No"])
                        obs_ventanas = st.text_input(f"Observaciones ventanas - obligatorio en caso de disconformidad",
                                                     key=f"obs_ventana_{i}")
                        st.write("")
                        puertas = st.pills(
                            f"**El área evaluada cuenta con puertas que proporcionan una vía adicional para la ventilación natural, al permitir la circulación del aire de forma controlada.**",
                            key=f"puertas_{i}", options=["Sí", "No"])
                        obs_puertas = st.text_input(f"Observaciones puertas - obligatorio en caso de disconformidad",
                                                    key=f"obs_puertas_{i}")
                        st.write("")
                        condiciones_disconfort = st.pills(
                            f"**El area presenta otras condiciones que pueden considerarse como causantes de disconfort térmico, que no hayan sido indicadas en los puntos anteriores.**",
                            key=f"cond_{i}", options=["Sí", "No"])
                        obs_condiciones = st.text_input(
                            f"Observaciones sobre disconfort térmico - obligatorio en caso de presencia",
                            key=f"obs_cond_{i}")
                ...
                st.markdown("---")
                st.write("Para finalizar las mediciones del centro de trabajo es necesario guardar la información completada")
                submitted = st.form_submit_button(label="Guardar información", type="primary", use_container_width=True, icon=":material/check_circle:")
                ...
                if submitted:
                    ...
                    datos_informe = {
                        "empresa": {
                            "Razón Social": campos_ct.get("razon_social", ""),
                            "RUT": campos_ct.get("rut", ""),
                            "CUV": campos_ct.get("cuv", ""),
                            "Nombre de Local": campos_ct.get("nombre_local", ""),
                            "Dirección": campos_ct.get("direccion", ""),
                            "Comuna": campos_ct.get("comuna", ""),
                            "Región": campos_ct.get("region", ""),
                        },
                        "datos_generales": {
            ...
            interpretation = interpreter_pmv(results.pmv)
            st.markdown(
                f"##### El valor de PMV {results.pmv} indica que la sensación térmica es: **{interpretation}**.")
